...
CELLS.append(
    md(
        ...
        "│   ├── db_retrieval.py    SQLite hit cache + cross-mode fuzzy match (speech)",
...
CELLS.append(
    md(
        ...
        "db_retrieval=True)`. Retrieval time is counted against the question's",
        "latency budget by design, since the 30 s timer measures",
        "end-to-end wall-clock latency rather than LLM-only.",
        "",
...
CELLS.append(
    md(
        ...
        "When the same question has already been seen in",
        "text mode, the Whisper output is fuzzy-matched against past option strings rather than trusted",
        "verbatim. The threshold is high enough to reject mishearings",
        "while tolerating casing and punctuation drift:",
        "",
        "```python",
        "# src/polimillionaire/strategies/db_retrieval.py  (excerpt)",
        "_FUZZY_THRESHOLD = 0.88  # SequenceMatcher ratio after normalisation",
        "",
        "def _fuzzy_resolve_option(question, cached_text):",
        "    # exact-after-normalise wins immediately; below-threshold abstains;",
        "    # ambiguous (>=2 options above threshold) also abstains.",
        "    ...",
        "```",
        "",
        "This substantially improves speech-mode accuracy on questions that have",
        "already been solved in text mode, even when Whisper mishears the question",
        "stem, provided the option text matches.",
    )
)
...
CELLS.append(
    code(
        ...
        "    select_cols = 'competition_id, model_name, strategy_name, prompt_version,' \\",
        "                  ' predicted_option_id, correct_option_id_if_known, latency_ms,' \\",
        "                  ' generated_answer' + (', mode' if has_mode else '')",
        "    df = pl.read_database(",
        "        f'SELECT {select_cols} FROM predictions'",
        ...
        "",
        "df = df.with_columns(",
        "    (pl.col('predicted_option_id') == pl.col('correct_option_id_if_known')).alias('correct'),",
        "    pl.col('competition_id').replace_strict(COMPETITION_NAMES, default='?').alias('competition'),",
        ")",
        "",
        "# Older DB snapshots predate the speech-mode migration; assume text-mode.",
        "if not has_mode:",
        "    df = df.with_columns(pl.lit('text').alias('mode'))",
        "",
        "leaderboard = (",
        ...
        "      )",
        "      .filter(pl.col('n') >= 20)  # drop combos with too few samples to be meaningful",
...
CELLS.append(
    code(
        ...
        "    cols = {r[1] for r in con.execute('PRAGMA table_info(predictions)').fetchall()}",
        "    has_mode = 'mode' in cols",
        "    mode_filter = \"AND mode = 'text'\" if has_mode else ''",
        "    cid_list = ','.join(str(c) for c in LIVE_ONLY_CIDS)",
        "    live_df = pl.read_database(",
        "        f'''SELECT competition_id, model_name, strategy_name, prompt_version,",
        "                   predicted_option_id, correct_option_id_if_known,",
        "                   latency_ms",
        "              FROM predictions",
        "             WHERE competition_id IN ({cid_list})",
        ...
        "",
        "if live_df.is_empty():",
        "    print('No server-confirmed cid=4 or cid=5 rows in this DB snapshot yet.')",
...
CELLS.append(
    code(
        ...
        "                db_retrieval=True, min_rerank_score=0.15,",
        "            )",
        "        return make_strategy(",
        "            'auto', llm, competition_id=cid, verbose=True,",
        "            max_steps=MAX_STEPS, min_rerank_score=0.15, db_retrieval=True,",
        "        )",
        "",
...
CELLS.append(
    md(
        "### Speech-mode play",
        "",
        "Same routing, with `mode='speech'` and `use_text_mode_retrieval=True`, so that the",
        "fuzzy DB cache accumulated during text-mode play assists speech runs, which",
        "would otherwise begin from scratch.",
    )
)
...
CELLS.append(
    code(
        ...
        "            db_retrieval=True, mode='speech', use_text_mode_retrieval=True,",
        "        )",
        "        if cid == 3:",
...
CELLS.append(
    md(
        ...
        "| Speech DB fallback | `src/polimillionaire/strategies/db_retrieval.py` |",
